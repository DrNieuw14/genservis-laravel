...
<div class="bg-white rounded-xl shadow-lg p-6 lg:p-8">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <x-back-button :href="route('project-estimates.show', $estimate->id)" />
    </div>
    ...
    <form method="POST" action="{{ route('project-estimates.update', $estimate->id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="project_name" value="{{ old('project_name', $estimate->project_name) }}"
                    class="w-full border rounded-lg p-4" required>
            </div>

            <div>
                <label class="block mb-2 font-semibold">Location (optional)</label>
                <input type="text" name="location" value="{{ old('location', $estimate->location) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            ...
            <div>
                ...
                <input type="text" name="duration" value="{{ old('duration', $estimate->duration) }}"
                    class="w-full border rounded-lg p-4">
            </div>
            ...
            <div>
                ...
                <select name="job_request_id" class="w-full border rounded-lg p-4">
                    ...
                    @foreach($jobRequests as $jr)
                        <option value="{{ $jr->id }}" {{ (int) old('job_request_id', $estimate->job_request_id) === $jr->id ? 'selected' : '' }}>
        ...
        <div class="mt-6">
            ...
            <textarea name="assumptions" rows="2" class="w-full border rounded-lg p-4">{{ old('assumptions', $estimate->assumptions) }}</textarea>
        </div>

        <div class="mt-6">
            <label class="block mb-2 font-semibold">Exclusions (optional)</label>
            <textarea name="exclusions" rows="2" class="w-full border rounded-lg p-4">{{ old('exclusions', $estimate->exclusions) }}</textarea>
        </div>
